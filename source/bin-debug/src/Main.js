...
var Main = (function (_super) {
    ...
    __egretProto__.onRunButtonClick = function (e) {
        egret.Tween.removeTweens(this.jian_png);
        var isloop = this.isloopCheckBox.selected; //是否循环
        var duration = this.durationSlider.getValue(); //耗时，毫秒。 2000=2秒完成
        var waited = 500; //下个动画等待时间 如果不循环，设置为0
        var ease = this.dropDownList.selectedItem.ease;
    ...
    __egretProto__.tweenComplete = function (param1, param2, param3) {
        //alert(param1);
        egret.gui.Alert.show(param1, "Alert");
    };
    __egretProto__.onResetButtonClick = function (e) {
        egret.Tween.removeTweens(this.jian_png);
        this.resetPosition();
    };
    __egretProto__.resetPosition = function () {
        this.jian_png.x = 80;
        this.jian_png.y = 160;
    ...
    __egretProto__.createChildren = function () {
        var runBtn = new egret.gui.Button();
        runBtn.name = "run";
        runBtn.label = "start";
        ...
        var resetBtn = new egret.gui.Button();
        resetBtn.name = "reset";
